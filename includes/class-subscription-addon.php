<?php
	...
	class WC_Addons_Gateway_Simplify_Commerce extends WC_Gateway_Simplify_Commerce {
		...
		public function __construct() {
			...
			if ( class_exists( 'WC_Subscriptions_Order' ) ) {
				...
				add_filter( 'woocommerce_subscription_payment_meta',
					array( $this, 'add_subscription_payment_meta' ), 10, 2 );
		...
		public function process_payment( $order_id ) {
			if ( ! isset( $_REQUEST['wc-simplifycommerce-card-token'] ) ) {
				return parent::process_payment( $order_id );
			}

			$order = wc_get_order( $order_id );

			$cart_token  = $_REQUEST['wc-simplifycommerce-card-token'] ?? null;
			$amount      = $_REQUEST['wc-simplifycommerce-amount'] ?? null;
			$order_total = $this->get_total( $order );

			if ( is_null( $cart_token ) && is_null( $amount ) ) {
				return [
					'result'   => 'fail',
					'redirect' => '',
				];
		...
		public function process_subscription_payment( $order, $amount = 0, $token = array() ) {
			...
			if ( $this->get_total_amount( $amount ) < 50 ) {
				return new WP_Error( 'simplify_error',
					__( 'Sorry, the minimum allowed order total is 0.50 to use this payment method.',
						'woocommerce-gateway-simplify-commerce' ) );
			}
			...
			if ( ! $customer_id ) {
				return new WP_Error( 'simplify_error',
					__( 'Customer not found', 'woocommerce-gateway-simplify-commerce' ) );
			}
			...
			try {
				...
				$data = array(
					...
					'description' => sprintf( __( '%s - Order #%s', 'woocommerce-gateway-simplify-commerce' ),
				...
				$order->add_order_note( sprintf( __( 'Gateway payment error: %s',
					'woocommerce-gateway-simplify-commerce' ), $error_message ) );
		...
		public function scheduled_subscription_payment( $amount_to_charge, $renewal_order ) {
			...
			if ( is_wp_error( $result ) ) {
				$renewal_order->update_status( 'failed',
					sprintf( __( 'Gateway Transaction Failed (%s)', 'woocommerce-gateway-simplify-commerce' ),
		...
		public function process_pre_order_release_payment( $order ) {
			try {
				...
				$pre_order_name = sprintf( __( '%s - Pre-order for "%s"', 'woocommerce-gateway-simplify-commerce' ),
						$order_item['name'] ) . ' ' . sprintf( __( '(Order #%s)',
						'woocommerce-gateway-simplify-commerce' ),
						$order->get_order_number() );
				...
				if ( 'APPROVED' == $payment->paymentStatus ) {
					...
					return new WP_Error( 'simplify_payment_declined',
						__( 'Payment was declined - the customer need to try another card.',
							'woocommerce-gateway-simplify-commerce' ) );
				}
			} catch ( Exception $e ) {
				$order_note = sprintf( __( 'Gateway Transaction Failed (%s)', 'woocommerce-gateway-simplify-commerce' ),
					$e->getMessage() );
