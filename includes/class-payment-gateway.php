<?php
	...
	class WC_Gateway_Simplify_Commerce extends WC_Payment_Gateway_CC {
		...
		public function init_form_fields() {
			$this->form_fields = array(
				...
				'integration_mode'    => array(
					...
					'type'        => 'select',
					'options'     => array(
						self::INTEGRATION_MODE_EMBEDDED => __( 'Embedded Form',
							'woocommerce-gateway-simplify-commerce' ),
		...
		/**
		 * Prints the Payment Form on checkout page
		 */
		public function payment_fields() {
			$this->payment_fields_description();
			$this->hosted_payments_form();
		}

		/**
		 * @return void
		 */
		protected function payment_fields_description() {
		 ...
		 * @return void
		 */
		protected function hosted_payments_form() {
			if ( $this->is_modal_integration_model ) {
				$this->hosted_payments_modal_option_form();
			} else {
				$this->hosted_payments_embedded_option_form();
			}
		}
		 ...
		 * @return array
		 */
		protected function get_hosted_payments_args( $cart ) {
			return apply_filters( 'woocommerce_simplify_commerce_hosted_args', array(
				'sc-key'    => $this->public_key,
				'amount'    => $this->get_total_amount( $cart->get_total( 'amount' ) ),
		 ...
		 * @return array
		 */
		protected function get_iframe_args() {
			$cart = WC()->cart;

			$args = $this->get_hosted_payments_args( $cart );
		 ...
		 * @return void
		 */
		protected function hosted_payments_embedded_option_form() {
			$iframe_args = $this->get_iframe_args();
			$public_key  = $this->public_key;
			?>

            <script type="text/javascript" src="https://www.simplify.com/commerce/simplify.pay.js"></script>
            <iframe name="embedded_pay" class="simplify-embedded-payment-form"
				<?php
					echo implode( ' ', $iframe_args ) ?>></iframe>

            <input type="hidden" name="wc-simplifycommerce-amount" value="">
            <input type="hidden" name="wc-simplifycommerce-card-token" value="">

            <script>
                var publicKey,
                    checkoutForm,
                    placeOrderButton,
                    ensureWpCheckoutView,
                    getWPCheckoutValue;
                ...
                jQuery(function ($) {
                    ...
                    getWPCheckoutValue = function (key) {
                        var values, valuesKey, i;

                        values = checkoutForm.serializeArray();

                        for (i = 0; i < values.length; i++) {
                            valuesKey = values[i]['name'] || null;
                            if (valuesKey === key) {
                                return values[i]['value'] || null;
                            }
                        }
                    };

                    ensureWpCheckoutView = function () {
                        var paymentMethod;
                        paymentMethod = getWPCheckoutValue('payment_method');

                        if (paymentMethod === "<?php echo $this->id ?>") {
                ...
                SimplifyCommerce.hostedPayments(
                    function (data) {
                        ...
                        checkoutForm.find("[name=wc-simplifycommerce-amount]").val(data.amount);
                        checkoutForm.find("[name=wc-simplifycommerce-card-token]").val(data.cardToken);
                        checkoutForm.submit();
                    },
                    {
                        scKey: publicKey
		...
		protected function hosted_payments_modal_option_form() {
			$iframe_args = $this->get_iframe_args();
			$public_key  = $this->public_key;
			?>
		...
		public function process_payment( $order_id ) {
			...
			$cartToken     = $_REQUEST['wc-simplifycommerce-card-token'] ?? null;
			$requestAmount = $_REQUEST['wc-simplifycommerce-amount'] ?? null;
			$orderAmount   = $this->get_total( $order );

			if ( is_null( $cartToken ) && is_null( $requestAmount ) ) {
				return [
					'result'   => 'fail',
					'redirect' => '',
				];
			...
			if ( (int) $requestAmount !== $orderAmount ) {
				return [
					'result'   => 'fail',
					'redirect' => '',
				];
			...
			if ( $this->txn_mode === self::TXN_MODE_PURCHASE ) {
				$order_complete = $this->purchase( $order, $orderAmount, $cartToken );
			...
			if ( $this->txn_mode === self::TXN_MODE_AUTHORIZE ) {
				$order_complete = $this->authorize( $order, $orderAmount, $cartToken );
		...
		protected function process_standard_payments( $order, $cart_token = '' ) {
			try {
				...
				$payment_response = $this->purchase( $order, $this->get_total( $order ), $pass_tokens );
		 ...
		 * @uses  Simplify_BadRequestException
		 */
		public function purchase( $order, $amount = 0, $token = array() ) {
			if ( ! is_array( $token ) ) {
			...
			try {
				...
				$order->add_order_note( sprintf( __( 'Gateway payment error: %s',
					'woocommerce-gateway-simplify-commerce' ),
					$error_message ) );

				return new WP_Error( 'simplify_payment_declined', $e->getMessage(),
		...
		/**
		 * @param WC_Order|WC_Cart $resource
		 *
		 * @return int
		 */
		protected function get_total( $resource ) {
			return $this->get_total_amount( $resource->get_total() );
		}
		 ...
		 * @return bool
		 */
		protected function authorize( $order, $amount, $cardToken ) {
			if ( (int) $amount !== $this->get_total( $order ) ) {
		 ...
		 * @return bool
		 */
		public function process_authorization_order_status(
			$order,
			$payment_id,
			$status,
			$auth_code,
		...
		public function process_refund( $order_id, $amount = null, $reason = '' ) {
			try {
				...
				if ( 'APPROVED' === $refund->paymentStatus ) {
					$order->add_order_note(
						sprintf(
							__( 'Gateway refund approved (ID: %s, Amount: %s)',
								'woocommerce-gateway-simplify-commerce' ),
							$refund->id,
							$amount
						)
